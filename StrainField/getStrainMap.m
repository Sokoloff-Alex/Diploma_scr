...
for iLong = 1:(size(VelGrid,1)-1)
   for iLat = 1:(size(VelGrid,2)-1)
       ...
       velA = squeeze(VelMap(iLong,     iLat)); 
       velB = squeeze(VelMap(iLong,     iLat + 1)); 
       velC = squeeze(VelMap(iLong + 1, iLat)); 
       StrainAB = getStrain2(pointA,velA, pointB,velB);
       StrainAC = getStrain2(pointA,velA, pointC,velC);
       Strain(iLong,iLat)   = StrainAC;
       Strain(iLong,iLat+1) = StrainAB;
       
       Grid(iLong,ILat) = LatGrid       
   end
end
