function [Strain] = getStrain3(pointA, pointB, pointC, velAB, velAC)
...
%%
    fxx = (norm(deg2km(0.25, 6378 * cosd(y1(i)) )*1000, norm(dx(i) , dy(i)) )  - deg2km(0.25, 6378 * cosd(y1(i)) )*1000) / deg2km(0.25, 6378 * cosd(Lat1(i)) )*1000; % ok
    fyy = (norm(deg2km(0.25)*1000, norm(dx(i) ,  dy(i)) ) - deg2km(0.25)*1000)/ deg2km(0.25)*1000;  % ok
    fxy = dx(i) / (deg2km(0.25)*1000 + dy(i)) ;   %  dVx/dy
    fyx = dy(i) / (deg2km(0.25, 6378 * cosd(y1(i)))*1000 + dx(i));   %  dVy/dx
