function [output] = solve_LSC_2(lat0, long0, lat, long, Vn, Ve, SigmaVn, SigmaVe, fType, varargin)
% solve Least Square Collocation
%
% input   :     lat0, long0     - coordinates of grid point , [deg]
%               lat, long       - coordinates of observation points
%               Vn Ve           - velocity components
%               SigmaVn, SigmaVe  sigma/sdt of velocity 
...
Sigma_V_pred = C_new' * C_obs^-1 * [SigmaVn; SigmaVe];
Sigma_V_pred = Sigma_V_pred';
...
if ismember('-v', flags)
    disp(['point ::', ... 
          ...
          '; sigmaVn = ',  num2str(Sigma_V_pred(1)),...
          '; sigmaVe = ',  num2str(Sigma_V_pred(2)) ]);
end

output = [V_pred, Sigma_V_pred];
