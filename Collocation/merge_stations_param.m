function [Value_ave,uniq_names, names_unified] = merge_stations_sum(Value,names, param)
    ...
    if ~isempty(iSet) 
        for j=1:length(iSet)
            ...
            switch param
                ...
                case {'sum','plus','add'}
                    Value_unified(row,:) = sum(Value(iSet,:));
                case 'mean'
                    Value_unified(row,:) = mean(Value(iSet,:));
            end
...
for i = 1:length(uniq_names)
    ...
    case 'sum'
        Value_ave(i,:) = sum(Value_unified(iSet,:));
    case 'mean'
        Value_ave(i,:) = mean(Value_unified(iSet,:));
    end
end
